...
- 简历管理：上传、替换、列表、详情、删除和原文件预览；支持 PDF、DOCX、PNG、JPG、JPEG。
...
- `/resumes`：简历上传、列表、详情、替换、删除和预览。
- `/model-providers`：模型供应商配置 CRUD。
- `/model-selections`：模型选择配置 CRUD。
- `/ai/chat`：同步 AI 对话。
- `/ai/chat/stream`：SSE 流式 AI 对话。

流式事件包括 `thread`、`token`、`tool_start`、`tool_end`、`tool_error`、`interrupt`、`final`、`error`。前端当前也支持展示 `reasoning` 类型事件。
