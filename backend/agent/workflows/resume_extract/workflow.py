...
from ...agents.resume_extractor import ResumeExtractorAgent, State
...
class ResumeExtractWorkflow(BaseWorkflow[Resume, State]):
    ...
    def _construct_initial_state(
        self, model: MaybeCallable[ModelSelection], resume_document: ResumeDocument
    ) -> State:
        return State(model=model, messages=[], resume_document=resume_document)
