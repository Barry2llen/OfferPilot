...
class ResumeExtractorAgent(BaseAgent[State]):
    ...
    async def _set_up_node(self, state: State) -> State:
        ...
        try:
            ...
            raise ResumePreviewConversionError(
                f"Failed to extract text from resume: {e}"
            )
        ...
        try:
            ...
            logger.debug(
                lambda: (
                    f"Converted resume document to images for image-based extraction. Number of images: {len(resume_images)}"
                )
            )
            _dispatch_custom_event_safely(
                "on_progress_update",
            ...
            raise ResumePreviewConversionError(
                f"Failed to convert resume to image: {e}"
            )
        ...
        while True:
            ...
            try:
                logger.debug(
                    lambda: (
                        "Invoking model for validation of the extracted resume text."
                    )
                )
                ...
                if validation.is_valid:
                    logger.debug(
                        lambda: (
                            f"Extracted resume text seems to be valid, reason: {validation.reason or 'No reason provided.'}"
                        )
                    )
                else:
                    logger.debug(
                        lambda: (
                            f"Extracted resume text seems to be invalid, reason: {validation.reason or 'No reason provided.'}"
                        )
                    )
        ...
        if not validation.is_valid:
            logger.debug(
                lambda: (
                    "Falling back to OCR-based extraction due to invalid text-based extraction."
                )
            )
            _dispatch_custom_event_safely(
                "on_progress_update",
            ...
            try:
                ...
                raise ResumePreviewConversionError(
                    f"Failed to extract text from resume using OCR: {e}"
                )
    ...
    async def _extract_section_node(self, state: State) -> State:
        ...
        logger.debug(lambda: "Extracting sections from the resume text.")
        _dispatch_custom_event_safely(
            "on_progress_update",
            ProgressUpdateEvent(
                progress=0.4,
        ...
        while True:
            ...
            try:
                logger.debug(
                    lambda: (
                        "Invoking model for extracting sections from the resume text."
                    )
                )
        ...
        return State(sections=sections.sections)
    ...
    async def _extract_facts_node(self, state: State) -> State:
        ...
        logger.debug(lambda: "Extracting facts from each resume section.")
        ...
        async def _extract_facts_for_section(
            section: ResumeSectionEx,
        ) -> ResumeSection | None:
            section_text = (
                f"[section_id]\n{section.section_id}\n\n"
                f"[title]\n{section.title}\n\n"
            ...
            while True:
                try:
                    logger.debug(
                        lambda: (
                            f"Invoking model for extracting facts from section: {section.title}"
                        )
                    )
                    await _adispatch_custom_event_safely(
                        "on_progress_update",
                    ...
                    logger.error(
                        f"Error calling model for section {section.title}:\n{e}"
                    )
                    await _adispatch_custom_event_safely(
                        "on_model_call_error",
                        ModelCallErrorEvent(
        ...
        while True:
            ...
            for _ in range(max_retries):
                results = await asyncio.gather(
                    *(_extract_facts_for_section(section) for section in sections)
                )

                sections_with_facts.extend(
                    [result for result in results if result is not None]
                )
                completed_sections = len(sections_with_facts)
                failed_sections = sum(1 for result in results if result is None)
                progress = (
                    0.65 + (0.3 * completed_sections / total_sections)
                    if total_sections
                    else 0.95
                )
                await _adispatch_custom_event_safely(
                    ...
                    await _adispatch_custom_event_safely(
                        ...
                        ProgressUpdateEvent(
                            ...
                            additional_data={
                                ...
                                "fact_count": sum(
                                    len(section.facts)
                                    for section in sections_with_facts
                                ),
                            },
                        ),
                    )
                ...
                logger.debug(
                    lambda: (
                        f"Error occurred while extracting facts for some sections: {[section.title for section in sections]}. Retrying for these sections, attempt {_ + 1}/{max_retries}."
                    )
                )
